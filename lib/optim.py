...
def tn_optimize(tensors, loss_function, eval_function, optimizer=Ranger, tol=1e-4, max_iter=1e6, print_freq=500, verbose=True, patience=10,lr=1e-2):
    """
    ...
    while True:
        ...
        if iter >= 2:
            if tol and (losses[-1] <= tol or -delta_loss / losses[-1] <= tol) and losses[-2] - \
                losses[-1] < losses[-3] - losses[-2]:
                converged_msg = 'Loss less than tolerance'
                converged = True
                break
            elif patience and iter - best_iter > patience:
                converged_msg = 'Eval loss stopped increasing'
                converged = True
                break
        if iter == max_iter:
            break
